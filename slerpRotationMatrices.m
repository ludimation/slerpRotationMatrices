...
secondTermSinc = sinc(rMag) * rx;
secondTermSinOverMag = (sin(rMag) / rMag) * rx;
...
% % using formulat from wikipedia -- http://en.wikipedia.org/wiki/Rodrigues'_rotation_formula#Conversion_to_rotation_matrix
% R = cos(rMag) * eye(3)                              ...    
%     + sin(rMag) * rx                                ...
%     + ( 1 - cos(rMag) ) .* ( r * r')                ; 
end
