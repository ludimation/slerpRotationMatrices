...
theta = AA( 4 );
rn = AA ( 1:3 );
% rn = rn * theta; % not sure why I was scaling this to begin with.
% rMag = norm( rn ); % TODO: should this actually be abs( theta )?
rMag = abs( theta ); % TODO: should this actually be abs( theta )?
rx = [                        ...
    0       -rn(3)	rn(2)   ; ...
    rn(3)   0       -rn(1)	; ...
    -rn(2)  rn(1)   0         ...
...
% TODO: this calculation seems to still be a bit off
R = cos(rMag) * eye(3)                              ...
    + (sin(rMag) / rMag) .* rx                      ...
    + ( ( 1 - cos(rMag) ) / rMag^2 ) .* ( rn * rn') ; 
end
